...
from .utils import (
    ...
    tdpo_kl_get_batch_logps,
    fdpo_kl_get_batch_logps,
    masked_mean,
    masked_var,
    entropy_from_logits,
    delete_dict,
...
class DPOTrainer(PairedPreferenceTrainer):
    ...
    def get_batch_metrics(self, batch: Dict[str, Union[List, torch.LongTensor]], mode: str=None):
        ...
        if self.reference_model is None:
            ...
            policy_chosen_logps, policy_rejected_logps, all_logits = self.forward(self.policy, batch)
            with torch.no_grad():
                reference_chosen_logps, reference_rejected_logps, reference_all_logits = self.forward(self.reference_model, batch)
            losses, chosen_rewards, rejected_rewards = self.loss(policy_chosen_logps, policy_rejected_logps, reference_chosen_logps, reference_rejected_logps)
        ...
        _, all_kl, _ = tdpo_get_batch_logps(all_logits, reference_all_logits, concatenated_batch['concatenated_labels'], average_log_prob=False)
        chosen_kl = all_kl[:batch['chosen_combined_input_ids'].shape[0]].detach()
        rejected_kl = all_kl[batch['chosen_combined_input_ids'].shape[0]:].detach()
        ...
        # kl
        metrics[f'kl_{mode}/chosen'] = chosen_kl.float().cpu().numpy().tolist()
        metrics[f'kl_{mode}/rejected'] = rejected_kl.float().cpu().numpy().tolist()
        metrics[f'kl_{mode}/margin'] = (chosen_kl - rejected_kl).float().cpu().numpy().tolist()
...
class SimPOTrainer(PairedPreferenceTrainer):
    def loss(self, chosen_logps_margin: torch.FloatTensor,
        ...
        return losses, chosen_rewards, rejected_rewards, (chosen_values*beta).detach(), (rejected_values*beta).detach()
    ...
    def get_batch_metrics(self, batch: Dict[str, Union[List, torch.LongTensor]], mode: str=None):
        ...
        chosen_logps_margin, rejected_logps_margin = self.forward(self.policy, batch)
        
        losses, policy_chosen_logps, policy_rejected_logps, chosen_rewards, rejected_rewards = self.loss(
...
class TDPO1Trainer(PairedPreferenceTrainer):
    ...
    def forward(self, model: nn.Module, batch: Dict[str, Union[List, torch.LongTensor]], average_log_prob=False) -> Tuple[torch.FloatTensor, torch.FloatTensor]:
        ...
        all_logits = model(concatenated_batch['concatenated_combined_input_ids'], attention_mask=concatenated_batch['concatenated_combined_attention_mask'], use_cache=(not self.is_mistral)).logits.to(self.policy_dtype)
        with torch.no_grad():
            reference_all_logits = self.reference_model(concatenated_batch['concatenated_combined_input_ids'], attention_mask=concatenated_batch['concatenated_combined_attention_mask'], use_cache=(not self.is_mistral)).logits.to(self.policy_dtype)

        all_logps_margin, all_position_kl, all_logps = tdpo_get_batch_logps(all_logits, reference_all_logits, concatenated_batch['concatenated_labels'], average_log_prob=False)
...
class TDPO2Trainer(TDPO1Trainer):
    def loss(self, chosen_logps_margin: torch.FloatTensor,
              rejected_logps_margin: torch.FloatTensor,
              chosen_position_kl: torch.FloatTensor,
              rejected_position_kl: torch.FloatTensor) -> Tuple[torch.FloatTensor, torch.FloatTensor, torch.FloatTensor]:
...
class TDPOKLTrainer(PairedPreferenceTrainer):
    ...
    def forward(self, model: nn.Module, batch: Dict[str, Union[List, torch.LongTensor]], average_log_prob=False) -> Tuple[torch.FloatTensor, torch.FloatTensor]:
        ...
        #     self.chosen_fm += reference_all_fm[:reference_all_fm.shape[0]//2, :, :].mean([0,1])
        #     self.rejected_fm += reference_all_fm[reference_all_fm.shape[0]//2:, :, :].mean([0,1])
        #     self.cnt += 1
        #     print(self.cnt)
        #     if self.cnt == 1000:
        #         self.chosen_fm /= self.cnt
        #         self.rejected_fm /= self.cnt
        #         torch.save(self.chosen_fm, '.cache/chosen_fm.pt')
        #         torch.save(self.rejected_fm, '.cache/rejected_fm.pt')
        #         exit()
        # else:
        #     self.chosen_fm = reference_all_fm[:reference_all_fm.shape[0]//2, :, :].mean([0,1])
        #     self.rejected_fm = reference_all_fm[reference_all_fm.shape[0]//2:, :, :].mean([0,1])
        ...
        all_logps_margin, all_position_kl, all_fm = tdpo_kl_get_batch_logps(
            all_logits, 
            reference_all_logits, 
            concatenated_batch['concatenated_labels'], 
            all_fm, 
            reference_all_fm, 
            use_mse=self.config.loss.use_mse,
        )
        ...
        chosen_position_kl = all_position_kl[:batch['chosen_input_ids'].shape[0]]
        rejected_position_kl = all_position_kl[batch['chosen_input_ids'].shape[0]:]
        chosen_fm_mse = all_fm[:batch['chosen_input_ids'].shape[0]]
        rejected_fm_mse = all_fm[batch['chosen_input_ids'].shape[0]:]
    ...
    def get_batch_metrics(self, batch: Dict[str, Union[List, torch.LongTensor]], mode: str=None):
        ...
        metrics[f'kl_{mode}/rejected'] = rejected_position_kl.float().detach().cpu().numpy().tolist()
        metrics[f'kl_{mode}/margin'] = (chosen_position_kl - rejected_position_kl).float().detach().cpu().numpy().tolist()
        metrics[f'kl_{mode}/fm margin'] = fm_mse.float().detach().cpu().numpy().tolist()
        metrics[f'kl_{mode}/fm chosen'] = chosen_fm_mse.float().detach().cpu().numpy().tolist()
        metrics[f'kl_{mode}/fm rejected'] = rejected_fm_mse.float().detach().cpu().numpy().tolist()
        metrics[f'loss/{mode}'] = all_devices_losses.float().cpu().numpy().tolist()

        del chosen_rewards, rejected_rewards, reward_accuracies, all_devices_losses

        return losses.mean(), metrics   

class FDPOKLTrainer(PairedPreferenceTrainer):
    def loss(self, chosen_fm_margin: torch.FloatTensor,
              rejected_fm_margin: torch.FloatTensor,
              chosen_fm_kl: torch.FloatTensor,
              rejected_fm_kl: torch.FloatTensor,
              chosen_logps_margin: torch.FloatTensor, 
              rejected_logps_margin: torch.FloatTensor,
        ):
        """Compute the TDPO loss for a batch of policy and reference model log probabilities.
        ...
        chosen_rejected_margin = chosen_fm_margin - rejected_fm_margin
        ...
        beta = self.config.loss.beta
        gamma = self.config.loss.gamma
        logits = beta * (chosen_rejected_margin - gamma)
        losses = -F.logsigmoid(logits - alpha*(rejected_fm_kl - chosen_fm_kl.detach()))
    ...
    def forward(self, model: nn.Module, batch: Dict[str, Union[List, torch.LongTensor]], average_log_prob=False) -> Tuple[torch.FloatTensor, torch.FloatTensor]:
        ...
        concatenated_batch = self.concatenated_inputs(batch)
        outputs = model(concatenated_batch['concatenated_combined_input_ids'], attention_mask=concatenated_batch['concatenated_combined_attention_mask'], use_cache=(not self.is_mistral))
        all_logits = outputs.logits.to(self.policy_dtype).detach()
        ...
        all_logps_margin, all_position_kl, all_fm_margin, all_fm = fdpo_kl_get_batch_logps(
            all_logits, 
            reference_all_logits, 
            concatenated_batch['concatenated_labels'], 
            all_fm, 
        ...
        chosen_fm_margin = all_fm_margin[:batch['chosen_input_ids'].shape[0]]
        rejected_fm_margin = all_fm_margin[batch['chosen_input_ids'].shape[0]:]

        return chosen_logps_margin, rejected_logps_margin, chosen_position_kl, rejected_position_kl, \
            chosen_fm_margin, rejected_fm_margin, chosen_fm_kl, rejected_fm_kl
    ...
    def get_batch_metrics(self, batch: Dict[str, Union[List, torch.LongTensor]], mode: str=None):
        ...
        chosen_logps_margin, rejected_logps_margin, chosen_position_kl, rejected_position_kl, chosen_fm_margin, rejected_fm_margin, chosen_fm_kl, rejected_fm_kl\
                = self.forward(self.policy, batch)
        ...
        losses, chosen_fm_reward, rejected_fm_reward, chosen_rewards, rejected_rewards = self.loss(
            chosen_fm_margin, 
            rejected_fm_margin,
            chosen_fm_kl, 
            rejected_fm_kl, 
        ...
        rejected_rewards = all_gather_if_needed(rejected_rewards, self.rank, self.world_size)
        reward_accuracies = all_gather_if_needed(reward_accuracies, self.rank, self.world_size)
        policy_chosen_logps = all_gather_if_needed(chosen_fm_reward.detach(), self.rank, self.world_size)
        policy_rejected_logps = all_gather_if_needed(rejected_fm_reward.detach(), self.rank, self.world_size)
        all_devices_losses = all_gather_if_needed(losses.detach(), self.rank, self.world_size)
...
class ORPOTrainer(PairedPreferenceTrainer):
    ...
    def forward(self, model: nn.Module, batch: Dict[str, Union[List, torch.LongTensor]], average_log_prob=False) -> Tuple[torch.FloatTensor, torch.FloatTensor]:
        ...
        return chosen_logps, rejected_logps, chosen_position_kl, rejected_position_kl
    ...
    def get_batch_metrics(self, batch: Dict[str, Union[List, torch.LongTensor]], mode: str=None):
        ...
        policy_chosen_logps, policy_rejected_logps, chosen_position_kl, rejected_position_kl = self.forward(self.policy, batch, average_log_prob=True)
        losses, chosen_rewards, rejected_rewards = self.loss(policy_chosen_logps, policy_rejected_logps)
